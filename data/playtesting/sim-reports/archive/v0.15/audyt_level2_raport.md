[Strona główna](../../../../../README.md) > [v0.15](README.md) > [audyt_level2_raport](audyt_level2_raport.md)

---

# Raport Audytu Poziomu 2 (Warunki Zwycięstwa i Skalowanie) — Wersja Balansu: v0.15

**Wersja Balansu:** `v0.15` | **Data:** 2026-08-14 12:05 | **Przeanalizowano Wariantów:** 28 | **Próba:** 2000 gier/setup | **Czas:** 113.77s
...
| `L2_SO_CONDEMNS_PLUS1` | Oficjum Skazania: 2/2/2 → 3/3/3 | 81.2 → 🟢 ** 77.9** (`-3.3`) | 90.5 → 91.5 (`⬆️ +1.0`) | 71.1 → 55.9 (`-15.2`) | 82.0 → 86.4 (`⬆️ +4.4`) | 🔴 POGARSZA GLOBALNIE |
| `L2_SO_CONDEMNS_MINUS1` | Oficjum Skazania: 2/2/2 → 1/1/1 | 81.2 → 🟡 ** 25.1** (`-56.1`) | 90.5 → 35.8 (`-54.7`) | 71.1 → 14.4 (`-56.7`) | 82.0 → 0.0 (`-82.0`) | 🔴 POGARSZA GLOBALNIE |
...
| `L2_CAA_ERA_PLUS1` | Cienie Era: 5/5/5 → 6/6/6 | 81.2 → 🟢 ** 81.0** (`-0.2`) | 90.5 | 71.1 → 70.7 (`-0.4`) | 82.0 → 81.7 (`-0.3`) | ⚪ OPTYMALNY |
| `L2_CAA_ERA_MINUS1` | Cienie Era: 5/5/5 → 4/4/4 | 81.2 → 🟢 ** 81.3** (`⬆️ +0.1`) | 90.5 | 71.1 → 71.3 (`⬆️ +0.2`) | 82.0 → 82.1 (`⬆️ +0.1`) | ⚪ OPTYMALNY |
| `L2_KB_ERA_PLUS1` | Korona Era: 6/6/6 → 7/7/7 | 81.2 → 🟡 ** 36.9** (`-44.3`) | 90.5 → 59.1 (`-31.4`) | 71.1 → 14.6 (`-56.5`) | 82.0 → 0.0 (`-82.0`) | 🔴 POGARSZA GLOBALNIE |
| `L2_KB_ERA_MINUS1` | Korona Era: 6/6/6 → 5/5/5 | 81.2 → 🟢 ** 90.3** (`⬆️ +9.1`) | 90.5 → 87.0 (`-3.5`) | 71.1 → 85.0 (`⬆️ +13.9`) | 82.0 → 98.8 (`⬆️ +16.8`) | 🟢 POPRAWIA GLOBALNIE |
| `L2_KB_DECREES_PLUS1` | Korona Dekrety: 2/2/2 → 3/3/3 | 81.2 → 🟡 ** 25.2** (`-56.0`) | 90.5 → 35.9 (`-54.6`) | 71.1 → 14.6 (`-56.5`) | 82.0 → 0.0 (`-82.0`) | 🔴 POGARSZA GLOBALNIE |
| `L2_KB_DECREES_MINUS1` | Korona Dekrety: 2/2/2 → 1/1/1 | 81.2 → 🟡 ** 25.2** (`-56.0`) | 90.5 → 35.9 (`-54.6`) | 71.1 → 14.6 (`-56.5`) | 82.0 → 0.0 (`-82.0`) | 🔴 POGARSZA GLOBALNIE |
| `L2_KB_HOOKS_PLUS1` | Korona Haki: 0/0/0 → 1/1/1 | 81.2 → 🟢 ** 50.0** (`-31.2`) | 90.5 → 70.8 (`-19.7`) | 71.1 → 29.3 (`-41.8`) | 82.0 → 0.0 (`-82.0`) | 🔴 POGARSZA GLOBALNIE |
| `L2_KT_FRAGS_PLUS1` | Kabała Fragmenty: 3/3/3 → 4/4/4 | 81.2 → 🟢 ** 59.0** (`-22.2`) | 90.5 → 88.5 (`-2.0`) | 71.1 → 40.3 (`-30.8`) | 82.0 → 48.3 (`-33.7`) | 🔴 POGARSZA GLOBALNIE |
| `L2_KT_FRAGS_MINUS1` | Kabała Fragmenty: 3/3/3 → 2/2/2 | 81.2 → 🟢 ** 69.1** (`-12.1`) | 90.5 → 88.9 (`-1.6`) | 71.1 → 65.2 (`-5.9`) | 82.0 → 53.1 (`-28.9`) | 🔴 POGARSZA GLOBALNIE |
