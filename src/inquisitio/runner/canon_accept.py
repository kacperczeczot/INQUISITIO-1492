...
def accept_global_candidate(
    ...
    *,
    ...
    Strictly accepts any candidate where delta_score >= min_delta without catastrophic failure.
    """
    safe, msg = telemetry_is_safe(cand, relax_era=True)
    if not safe:
    ...
    cand_vit = cand.get("vitality_penalty", 0.0)
    if cand_vit > 0.15 and d < 1.0:
        warns = cand.get("vitality_warnings", [])
        warn_str = "; ".join(warns[:2]) + ("..." if len(warns) > 2 else "")
        return AcceptDecision(False, f"Global: naruszenie witalności (kara {cand_vit:.3f} > 0.15) - {warn_str}", "global")

    # Zysk globalny — akceptujemy KAŻDĄ poprawkę podnoszącą wynik gry!
    if d >= min_delta and dmin >= -1.0:
        return AcceptDecision(True, f"Global Δscore {d:+.2f} ≥ {min_delta} (dmin {dmin:+.2f})", "global")
    if d >= 0.10:
        return AcceptDecision(True, f"Global Zysk Δscore {d:+.2f} (dmin {dmin:+.2f})", "global")
        
    return AcceptDecision(False, f"Global Δscore {d:+.2f} i Δmin {dmin:+.2f} nie dają zysku balansu", "global")
