[Strona główna](../../README.md) > [Gra](../README.md) > [Komponenty](README.md)

---

# Lista elementów do druku 3D — prototyp playtest

Komplet pod pierwszą testową grę (**3–5 graczy**).  
Karty, plansza i planszetki = **druk papierowy** (nie tu).  
Źródło ilości: [`inventory.md`](inventory.md) · zasady: [`../../docs/rules/ksiega.md`](../../docs/rules/ksiega.md).

---

## Priorytet 1 — obowiązkowe

### Wielki Inkwizytor (NPC)

| Element | Ilość | Uwagi |
| :--- | ---: | :--- |
| Figurka Wielkiego Inkwizytora | **1** | Wyższa niż Agent (~40–50 mm); baza z miejscem na żeton stanu **Patrol / Autodafé** |

Silueta: mitra / krzyż / płaszcz. Czytelna z daleka na planszy A2.

### Agenci (figurki pionków)

**3 na frakcję** × **5 frakcji** = **15 szt.**

| # | Frakcja | Kolor (sugerowany) | Silueta / motyw |
| :---: | :--- | :--- | :--- |
| 1–3 | Święte Oficjum | czerwień / czerń | inkwizytor, familiariusz, kaptur + krzyż |
| 4–6 | Cienie Al-Andalus | zieleń / turkus | asasyn, brat w kapturze, sztylet |
| 7–9 | Korona & Borgiowie | złoto / fiolet | dworzanin, kapitan straży, herb |
| 10–12 | Kabała z Toledo | błękit / srebro | uczony, alchemik, księga / gwiazda |
| 13–15 | Gildia Cieni | szarość / czerń matowa | złodziej, szantażysta, maska |

Wymagania: wysokość ~25–35 mm, baza okrągła z kolorem frakcji, **5 wyraźnie różnych siluet**.

### Nakładki Podwójny Agent

| Element | Ilość | Uwagi |
| :--- | ---: | :--- |
| Pierścień / klips na bazę Agenta | **8** | Oznacza Podwójnego; zdejmowalny; nie zasłania koloru frakcji |

### Znaczniki Herezji (płomienie)

| Element | Ilość |
| :--- | ---: |
| Marker płomienia / mały pionek na Tor Herezji | **5** (1 na gracza) |

---

## Priorytet 2 — żetony (zalecane w 3D)

Wspólny kształt PnP / 3D: **kwadrat 20×20 mm, lekko zaokrąglone rogi** (Agenci zostają na okrągłej bazie).

| Element | Ilość | Kształt / uwagi |
| :--- | ---: | :--- |
| **Hak** | **12** | **Jeden typ** — hak / łańcuch; ofiarę oznacza się kolorem / żetonem obok |
| Relikwia | **8** | kafelek; 1–2 warianty wyglądu OK |
| Fragment Kodeksu | **6** | fragment pergaminu (Kabała) |
| Złoto | **40** | kafelek ze znakiem monety; opcjonalnie 2 wartości (1 i 5) |
| Stos / Autodafé | **4** | płomień + stos / krzyż inkwizycji |
| Upadek frakcji (Gildia) | **4** | czarna pieczęć / zbita tarcza |
| Dekret (Korona) | **4** | pieczęć / pergamin (postęp karty specjalnej) |
| Zużycie Ery | **15** | mały kafelek ✕ — limit Nasłanie / Hak / Przesłuchanie |
| Stan Inkwizytora | **2** | małe kafle: Patrol / Autodafé (jeśli nie na bazie figurki) |

---

## Priorytet 3 — opcjonalne (nice to have)

| Element | Ilość | Po co |
| :--- | ---: | :--- |
| Zapasowi Agenci | **+5** (1 na frakcję) | zgubienie / playtest |
| Marker pierwszego gracza | **1** | zegar / pieczęć Toledo |
| Marker Szlaku Morskiego | **1** | statek / kotwica (Flota Kolumba) |
| Stojak Relikwii w Lochach | **1** | wyróżnienie lokacji startowej |
| Żetony „głos Werdyktu” | **10** | Skazać / Uniewinnić (opcjonalnie tajne głosowanie) |

---

## Nie drukować w 3D (papier / PnP)

- Plansza 5 lokacji (**A2** lub 4×A4)
- Planszetki gracza ×5 (Tor Herezji)
- Karty frakcji (10 × 5) + Talia Czasu (8+)
- Teach sheet / cele zwycięstwa

---

## Podsumowanie ilości (komplet 5p)

| Grupa | Sztuk |
| :--- | ---: |
| Wielki Inkwizytor | 1 |
| Agenci | 15 (+5 opcjonalnie) |
| Nakładki Podwójny | 8 |
| Herezja | 5 |
| Haki | 12 |
| Relikwie | 8 |
| Fragmenty | 6 |
| Złoto | 40 |
| Stosy | 4 |
| Upadek | 4 |
| Dekret | 4 |
| Opcjonalne markery | 2–5 |
| **Razem (P1+P2)** | **~107** |

### Minimalny zestaw na pierwszą sesję 3p

Tylko frakcje przy stole (np. Oficjum + Al-Andalus + Korona):

- Inkwizytor **1**, Agenci **9**, Podwójny **4**, Herezja **3**
- Haki **8**, Relikwie **5–6**, Fragmenty **4**, Złoto **25**, Stosy **2**, Upadek **2**, Dekret **2**

---

## Estetyka

Pixel art / mat: głęboka czerwień, złoto, czerń, pergamin.  
Referencje klimatu: *Blasphemous*, ciężki horror instytucjonalny.  
Final rzeźba dopiero po freeze tekstów (`docs/roadmap.md`).
