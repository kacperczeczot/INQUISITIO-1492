...
def apply_mutation_to_5p_config(raw_cfg: dict[str, Any], rule_params: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    def _set_5p(section_dict: dict, key: str, default_val: Any, offset: int, desc_name: str):
        cur = section_dict.get(key, default_val)
        if isinstance(cur, dict):
            base_v = cur.get("5p", cur.get("4p", default_val))
...
class AutoBalancer5P:
    ...
    def run(self):
        ...
        print(f"Dziennik operacji 5P:       {LOG_FILE_PATH}")
        print("═══════════════════════════════════════════════════════════════════════\n")
