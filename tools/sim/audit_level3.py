...
def build_level3_tests(param_filter: str = "all", faction_filter: str = "all", card_filter: str | None = None):
    ...
    for cid in sorted_card_ids:
        ...
        for p in params_to_test:
            ...
            name_p = f"{cid.upper()} ({c.name}): {p} {curr_val+1} (+1)"
            ...
            if curr_val > 0:
                ...
                name_m = f"{cid.upper()} ({c.name}): {p} {curr_val-1} (-1)"
...
def main():
    ...
    for rule_id, rule_name, rule_params in tests:
        t_rule = time.time()
    ...
    report_lines = [
        ...
        f"**Wynik Bazy Poziomu 3 (Global):** `{base['global_score']:.1f} pkt` | 3p: `{base['cat_scores'].get('3p',0.0):.1f} pkt` | 4p: `{base['cat_scores'].get('4p',0.0):.1f} pkt` | 5p: `{base['cat_scores'].get('5p',0.0):.1f} pkt`",
        "",
